...
function respond(instance, data, options) {
  const errorPrefix = this.app.lang.t('cannotsendresponse');
  ...
  if(!response || !response.status || !response.send) {
    return logError(instance, `${errorPrefix}, ${this.app.lang.t('invalidresponse')}`);
  }
  if(!data.statusCode) {
  ...
  if(instance.sendHtml) {
    if(!options.filepath) {
      return logError(this, `${errorPrefix}, ${this.app.lang.t('nofilepath')}`);
    }
