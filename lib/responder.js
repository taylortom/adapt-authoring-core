...
function respond(instance, data, options) {
  const errorPrefix = instance.app.lang.t('cannotsendresponse');
  if(!options.statusCode) {
    return logError(this, errorPrefix);
  }
  const response = instance.response;

  if(!response || !response.status || !response.send) {
    return logError(instance, `${errorPrefix}, ${instance.app.lang.t('invalidresponse')}`);
  }
  if(!data.statusCode) {
  ...
  if(instance.sendHtml) {
    if(!options.filepath) {
      return logError(this, `${errorPrefix}, ${instance.app.lang.t('nofilepath')}`);
    }
    return response.render(options.filepath, data);
  }

  function logError(instance, ...message) {
