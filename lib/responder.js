...
function respond(instance, data, options) {
  if(!options.statusCode) {
    return logError(this, 'Cannot send response, options.statusCode required');
  }
  const response = instance.response;

  if(!response || !response.status || !response.send) {
    return logError(instance, 'Cannot send response, invalid response object set');
  }
  if(!data.statusCode) {
  ...
  if(instance.sendHtml) {
    if(!options.filepath) {
      return logError(this, 'Cannot send response, options.filepath required');
    }
