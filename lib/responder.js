...
class Responder {
  ...
  * @return {Responder} This instance
  */
  json() {
    this.sendJson = true
    this.sendHtml = false;
...
function respond(instance, data, options) {
  const t = key => instance.app.lang.t(`error.${key}`);
  const log = (...message) => instance.app.logger.error(instance.name, ...message);
  const errorPrefix = t('cannotsendresponse');

  if(!options.statusCode) {
    return log(`${errorPrefix}, ${t('nostatuscode')}`);
  }
  const response = instance.response;

  if(!response || !response.status || !response.send) {
    return log(`${errorPrefix}, ${t('invalidresponse')}`);
  }
  ...
  if(instance.sendHtml) {
    if(!options.filepath) {
      return log(`${errorPrefix}, ${t('nofilepath')}`);
    }
