...
class DependencyLoader extends Events {
  ...
  */
  async getDependencies() {
    ...
    return globs.map(d => d.replace(`${path.sep}adapt.json`, ''));
  }
  /**
  ...
  */
  async loadModule(modName) {
    ...
    const tID = setTimeout(() => this.logError(`Load of module '${modName}' timed out`), 10000);
  ...
  */
  logError(message) {
    if(this.app.logger._isReady) this.app.logger.log('error', this.name, message);
