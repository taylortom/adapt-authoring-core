...
class DependencyLoader extends Events {
    ...
    Object.keys(this.dependencies).forEach(d => {
      try {
        ...
        if(c.adapt_authoring.utility && !this.utilities.hasOwnProperty(c.adapt_authoring.utility)) {
          this.utilities[c.adapt_authoring.utility] = this.createUtility(d,c);
        }
      } catch(e) {
  ...
  */
  initialiseModules() {
    try {
      Object.values(this.modules).forEach(m => typeof m.initialise === 'function' && m.initialise());
    } catch(e) {
      this.handleError(e);
    }
  ...
  initialiseUtilities(utils) {
    utils.forEach(u => {
      ...
      this.app[u] = this.utilities[u];
      typeof this.app[u].initialise === 'function' && this.app[u].initialise();
    });
  }
  /**
