...
class DependencyLoader extends Events {
  ...
  */
  async load() {
    ...
    const sorted = Object.values(this.configs).reduce((m,c) => {
  ...
  */
  async loadModule(modName) {
    ...
    const modDeps = config.moduleDependencies || [];
    const missingDeps = modDeps.filter(m => !this.configs[m]);
    if(missingDeps.length) {
      throw new Error(`${modName} requires the following missing dependencies: ${missingDeps.join(', ')}`);
    }
    ...
    return new Promise((resolve, reject) => {
      const tID = setTimeout(() => reject(`Load of module '${modName}' timed out`), config.loadTimeoutMS || 10000);
