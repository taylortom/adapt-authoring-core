...
class DependencyLoader extends Events {
  ...
  */
  async load() {
    ...
    const sorted = Object.values(this.dependencies).reduce((m,c) => {
    ...
    try { await this.loadModules(sorted.theRest); } catch(e) {}
    // TODO: check for failed modules + warn about any modules that depend on them
  }
  /**
  ...
  */
  async loadConfigs() {
    const files = await globPromise('node_modules/**/adapt.json');
    const deps = files.map(d => d.replace('/adapt.json', ''));
    await Promise.all((deps).map(async d => {
      try {
        const c = await this.loadConfig(d);
  ...
  */
  async loadConfig(relPath) {
    ...
    return {
      ...(await fs.readJson(path.join(absPath, 'package.json'))),
      ...(await fs.readJson(path.join(absPath, 'adapt.json'))),
      rootDir: absPath
    };
  }
  /**
  ...
  */
  async loadModules(modules) {
    const results = await Promise.allSettled(modules.map(async d => {
      try {
