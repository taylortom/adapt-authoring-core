...
class App extends Module {
  ...
  */
  ...
  initialiseDependencies() {
    const requiredUtils = ['config', 'lang', 'logger'];
    // create getters/setters for the following:
    requiredUtils.forEach(m => {
      Object.defineProperty(this, m, {
        get: () => this[`__${m}`],
        set: (val) => {
          if(this[m]) return this.log('warn', `${m} has already been set`);
          /** @ignore */
          this[`__${m}`] = val;
        }
      });
    ...
    */
    ...
    // check we've got the dependencies we need;
    const missingUtils = requiredUtils.filter(m => !this[m]);
    if(missingUtils.length) {
      this.log('error', `Missing the following utilities: ${missingUtils.join(', ')}`);
      hasErrored = true;
    }
    if(hasErrored) {
