...
class App extends AbstractModule {
  ...
  */
  ...
  logErrors(errors) {
    const t = this.lang && this.lang.t || function(k,d) { return `${k}: ${JSON.stringify(d)}` };
    errors.forEach(e => this.log(e.type, t(e.key, e.data)));
  }
  /**
  * Starts the app
  */
  async start() {
    try {
      ...
      this.log('success', this.lang.t('info.appstart', { dir: this.getConfig('root_dir') }));
    } catch(e) {
      this.log('error', `${this.lang.t('error.appstart')}: ${e}\n${e.stack}`);
    }
  }
}
